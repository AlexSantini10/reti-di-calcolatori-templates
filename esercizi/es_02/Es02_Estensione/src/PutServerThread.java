...
public class PutServerThread extends Thread {
    ...
    public PutServerThread(Socket clientSocket) {
        this.clientSocket = clientSocket;
    }

    public void run() {
        System.out.println("Attivazione figlio PUT: " + Thread.currentThread().getName());
        ...
        try {
            try {
                ...
                // Ricezione nome directory
                nomeDirectory = inSock.readUTF();

                fileCorr = new File(nomeDirectory);
                ...
                while ((nomeFileRicevuto = inSock.readUTF()) != null) {
                    ...
                    fileCorr = new File(nomeDirectory + "/" + nomeFileRicevuto);
                    if (fileCorr.exists()) {
                        outSock.writeUTF("salta file");
                    } else {
                        outSock.writeUTF("attiva");
                        numeroByte = inSock.readLong();
                        System.out.println("Scrivo il file " + nomeDirectory + "/" + nomeFileRicevuto + " di " + numeroByte + " byte");
                        outFileCorr = new FileOutputStream(nomeDirectory + "/" + nomeFileRicevuto);
