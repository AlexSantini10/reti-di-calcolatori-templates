...
int main(int argc, char *argv[])
{
    ...
    char buffer_message[MAX_BUFFER_SIZE];
    ...
    while (1)
    {
        ...
        if (received_bytes > 0)
        {
            ...
            // Controllo se il messaggio è OK
            if (strcmp(buffer, "0") == 0)
            {
                printf("Numero di patente aggiurnato correttamente\n");
            }
            else if (strcmp(buffer, "-1") == 0)
            {
                printf("Numero di patente non aggiornato\n");
            }
            else
            {
